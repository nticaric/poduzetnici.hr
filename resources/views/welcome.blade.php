...
<x-app-layout>
    ...
    <div class="relative -mt-16 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 z-10">
        ...
        ['name' => 'Poduzetnici', 'color' => 'text-purple-600', 'route' => 'profiles.index', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />'],
        ['name' => 'Alati', 'color' => 'text-indigo-600', 'route' => 'tools.index', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />'],
        ['name' => 'Oglasi', 'color' => 'text-orange-600', 'route' => 'ads.index', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />'],
        ['name' => 'Oglasni prostor', 'color' => 'text-pink-600', 'route' => 'advertising.index', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 017 6h1.832c4.1 0 7.625-1.234 9.168-3v14c-1.543-1.766-5.067-3-9.168-3H7a3.988 3.988 0 01-1.564-.317z" />'],
        ['name' => 'Pitanja i odgovori', 'color' => 'text-teal-600', 'route' => 'faq', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />'],
    ] as $cat)
                <a href="{{ isset($cat['route']) ? route($cat['route']) : route('ads.index', ['category' => $cat['name']]) }}" class="bg-white border border-gray-100 p-4 sm:p-6 rounded-xl sm:rounded-2xl shadow-lg hover:shadow-2xl hover:-translate-y-1 transition-all group text-center block">
                    <div class="{{ $cat['color'] }} mb-2 sm:mb-3 group-hover:scale-110 transition-transform inline-block p-2 sm:p-3 rounded-full bg-gray-50 group-hover:bg-white">
                        <svg class="w-6 h-6 sm:w-8 sm:h-8 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">{!! $cat['icon'] !!}</svg>
                    </div>
                    <h3 class="text-dark-900 font-bold text-sm sm:text-lg group-hover:text-primary-600 transition-colors">{{ $cat['name'] }}</h3>
                </a>
            @endforeach
        </div>
    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-20">
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4 mb-8">
            <div>
                <h2 class="text-2xl sm:text-3xl font-bold font-display text-dark-900">Najnovije prilike</h2>
                <p class="text-gray-500 mt-1 sm:mt-2 text-sm sm:text-base">Pregledajte najnovije oglase iz naše zajednice</p>
            </div>
            <a href="{{ route('ads.index') }}" class="text-primary-600 font-semibold hover:text-primary-700 flex items-center text-sm sm:text-base">
